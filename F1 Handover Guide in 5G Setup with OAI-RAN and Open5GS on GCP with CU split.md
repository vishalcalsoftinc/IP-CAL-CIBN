...
## **F1 Handover Guide of 5G Setup with OAI-RAN and Open5GS on GCP with CU split**
...
sequenceDiagram
    ...
    Note over CU-CP, UE: Initial State: Network is up.<br/>UE is attached and has an active PDU session via Source DU (Cell ID bc614e).
    ...
    Note over CU-CP: Handover triggered for UE (RNTI 5200) to Target DU (Cell ID bc614f)
    ...
    Note over UE: UE receives command, detaches from Source DU (Cell ID bc614e).<br/>Starts searching for Target DU (Cell ID bc614f).
    
    loop Synchronization & Random Access on Target Cell
        UE->>Target DU (DU1): PHY: Synchronization Signals (Finds Cell ID bc614f)
